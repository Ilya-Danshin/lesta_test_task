...
'''
Сортировка слиянием
    Сложность O(nlog(n)), при этом всегда, что лучше, чем быстрая сортировка,
    которая может деградировать до O(n^2) при плохом выборе разделяющего элемента.
    Однако на отсортированном массиве будет работать столько же, сколько и на не
    отсортированном - O(nlog(n)).
    Так как специфичных условий не было поставленно, результата лучше, чем O(nlog(n)) не добиться.
'''
class MergeSort:
    @staticmethod
